 ...
 *
 * Only the *form* half is here. Prices live in `data/fees.ts` and openings in
 * the `event_openings` and `event_switches` tables, so no file can ever
 * disagree with what somebody was actually charged or with what the committee
 * has actually opened.
 */

import { territories, type SubEvent, type Territory } from './territories.ts'
